...
class PluginFileAnalyzerWithInfoFile(IPluginFileAnalyzer):
    ...
    This file must contain at least the following information::
        ...
        Version = Major.minor.minorminor
    ...
    def getPluginNameAndModuleFromStream(self, infoFileObject, candidate_infofile=None):
        ...
        if not config_parser.has_section("Core"):
            log.error("Plugin info file has no 'Core' section (in '%s')" % candidate_infofile)
            return (None, None, None, None)
        if not config_parser.has_option("Core","Name") or not config_parser.has_option("Core","Module") or not config_parser.has_option("Core","Version"):
            log.error("Plugin info file has no 'Name', 'Module' or 'Version' section (in '%s')" % candidate_infofile)
            return (None, None, None, None)
        # check that the given name is valid
        name = config_parser.get("Core", "Name")
        name = name.strip()
        if PLUGIN_NAME_FORBIDEN_STRING in name:
            log.error("Plugin name contains forbiden character: %s (in '%s')" % (PLUGIN_NAME_FORBIDEN_STRING,
                                                                                 candidate_infofile))
            return (None, None, None, None)
        return (name, config_parser.get("Core", "Module"), config_parser.get("Core", "Version"), config_parser)
    ...
    def _extractCorePluginInfo(self,directory, filename):
        ...
        if not (isinstance(filename, str) or isinstance(filename,unicode)):
            ...
            name, moduleName, version, config_parser = self.getPluginNameAndModuleFromStream(filename)
        else:
            candidate_infofile = os.path.join(directory, filename)
            # parse the information file to get info about the plugin
            name, moduleName, version, config_parser = self.getPluginNameAndModuleFromStream(open(candidate_infofile),candidate_infofile)
        if (name, moduleName, version, config_parser) == (None, None, None, None):
            return (None,None)
        infos = {"name":name, "path":os.path.join(directory, moduleName), "version":version}
    ...
    def getInfosDictFromPlugin(self, dirpath, filename):
        ...
        if(infos):
            ...
            if infos.get("version", None) is None:
                raise ValueError("Missing *version* of the plugin in extracted infos.")
        return infos, config_parser
...
class PluginFileAnalyzerMathingRegex(IPluginFileAnalyzer):
    ...
    def getInfosDictFromPlugin(self, dirpath, filename):
        ...
        cf_parser.set("Core","Version",infos["version"])
...
#					print plugin_info.path
                        ...
                        if os.path.isdir(plugin_info.path):
                            ...
                            if (plugin_info.path.endswith(".py")):
                                to_display = plugin_info.path
                            else:
                                to_display = plugin_info.path+".py"
                            log.error("Module '%s' is missing!!" % (to_display))
                            break
